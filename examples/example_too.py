...
# <minidb-too>/examples/example_too.py


import datetime


# use this packages version
import os.path
from sys import path as _PATH
MODPATH = os.path.join(os.path.split(os.path.split(__file__)[0])[0], 'src')
_PATH.insert(0, MODPATH)
#print('__file__', __file__)


import minidb2 as minidb


class Person(minidb.Model):
    # Database columns must be annotated
    username:str=None
    mail:str=None
    foo:int=5

    # Not persisted (runtime-only) class attributes start with underscore
    _not_persisted:float = None
    _foo:object = None
    ...
    # This is the custom constructor that will be called by minidb2.
