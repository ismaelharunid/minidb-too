...
# <minidb-too>/src/minidb2/core.py
#           _      _    _ _
#     _ __ (_)_ _ (_)__| | |__
#    | '  \| | ' \| / _` | '_ \
...
from functools import partial as _FT_partial, wraps as _FT_wraps
from types import FunctionType


from .cli_helpers import ChoiceQuery
...
DEV_QUERY = ChoiceQuery('Continue (Yes/no/always): {default_withbs}',
                        persistent='always')
...
def _get_all_defaults(class_, include_private=False):
    for clazz in reversed(inspect.getmro(class_)):
        if hasattr(clazz, '__minidb_defaults__'):
            for name, default in clazz.__minidb_defaults__.items():
                if include_private or not name.startswith('_'):
                    yield (name, default)
...
class Store(object):
    ...
    def query(self, class_, select=None, where=None, order_by=None, group_by=None, limit=None):
        with self.lock:
            ...
            if isinstance(select, FunctionType):
            ...
            if group_by is not None:
                if isinstance(group_by, FunctionType):
                    # Late-binding of columns
...
def model_init(self, *args, **kwargs):
    ...
    defaults = dict(_get_all_defaults(self.__class__, include_private=True))
    ...
    for key, type_ in slots:
        _set_attribute(self, key, type_,
                       kwargs.get(key, defaults.get(key, None)))
...
class MetaModel(type):
    ...
    def __new__(mcs, name, bases, mixins):

        # Redirect __init__() to __minidb_init__()
        if '__init__' in mixins:
            mixins['__minidb_init__'] = mixins['__init__']
        mixins['__init__'] = model_init

        # Caching of live objects
        mixins['__minidb_cache__'] = weakref.WeakValueDictionary()

        slots = OrderedDict(
                (k, v) for k, v in mixins.items()
                if not (k.lower() != k
                        or k.startswith('__')
        ...
        if '__annotations__' in mixins:
            ...
            slot_defaults = dict((k, slots[k])
                                 ...
                                 if k in slots)
            slots.update(persistent_slots)
        else:
            persistent_slots = slots
            slot_defaults = {}

        keep = OrderedDict((k, v) for k, v in mixins.items()
                           if k not in persistent_slots)
        keep['__minidb_slots__'] = persistent_slots
        keep['__minidb_defaults__'] = slot_defaults
        ...
        if bases:
            print('name', name)
            print('__minidb_slots__', result.__minidb_slots__)
            print('__minidb_defaults__', result.__minidb_defaults__)
            if (q := DEV_QUERY()) not in ('yes', 'always'):
                print('q', q)
                exit(-1)
...
def pformat(result, color=False):
    ...
    inred, ingreen, inyellow, inblue = (_FT_partial(incolor, x) for x in range(1, 5))
